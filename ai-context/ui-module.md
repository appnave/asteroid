...
## Store-Adapter (Integração com State Management)

O `@bildvitta/store-adapter` abstrai Vuex/Pinia. Componentes usam:

```js
import { getAction, getState } from '@bildvitta/store-adapter'

// Fetch
getAction.call(this, { entity: 'users', action: 'fetchList', payload })

// Read state
getState.call(this, { entity: 'users', key: 'list' })
```

**Entity-based:** Cada view component declara um `entity` que mapeia para um módulo de store. O store-adapter resolve as actions (`fetchSingle`, `fetchList`, `create`, `update`, `replace`, `destroy`) automaticamente.
